...
```{r}
# Filter out people with valid birthYear and living (i.e., deathYear is NA)
...
```{r}
#creating table for display of resutls
mark_hamill_top_4_tb <- mark_hamill_top_4 |>
  gt() |>
  tab_header(
    title = "Mark Hamill's Top 4 Projects"
  ) |>
  cols_label(
...
```{r}
series_avg_ratings <- series_episode_count |>
